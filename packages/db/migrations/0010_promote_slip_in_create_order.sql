...
-- Phase 4 step 3: CREATE OR REPLACE create_order_with_items() RPC to promote
-- the slip session in the same transaction as inserting orders + order_items.
--
-- 3 new logic blocks vs migration 0006:
--   1. Validate uploadSessionId (lock + owner + status='uploaded') OR
--      reject with slip_required_for_transfer if paymentMethod='transfer' has none.
--   2. INSERT orders includes slip_key + slip_state derived from session.
--   3. UPDATE pending_slip_uploads.status='promoted' + INSERT order_slip_events('uploaded').
--
-- SECURITY INVOKER unchanged — orders RLS still enforced.
...
BEGIN
  ...
  IF v_showroom_id IS NULL THEN
    -- Coordinators have NULL showroom_id (oversee all). Fall back to client-provided
    -- or alphabetically-first showroom (multi-showroom safe-default).
