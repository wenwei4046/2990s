-- 0035_slip_required_all_methods.sql
-- Make a payment slip / proof compulsory for EVERY payment method, not just
-- bank transfer. Body unchanged from 0034 except the slip-gate branch:
-- the old `ELSIF (paymentMethod='transfer')` becomes a plain `ELSE`, so any
-- order submitted without a valid uploaded slip session is rejected with
-- `slip_required` regardless of method. POS already gates this client-side
-- (validateConfirmPayment); this is the server-side backstop. Legacy orders
-- placed before this migration are untouched.
