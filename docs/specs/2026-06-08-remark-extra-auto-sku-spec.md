...
| D6 | **SKU code** = `{MODEL}-{COMPARTMENT}-{REMARK_SLUG}` (uppercased), where REMARK_SLUG is auto-derived from the remark text. **Collision → append `-2`, `-3`, …** |
...
| D8 | **Re-activation (Phase 2)** makes it selectable again in POS. Sofa: via the **Allowed Options** panel → `allowed_options.compartments` + `sofaCompartmentMeta` + a **palette extension** that reuses the base compartment's PNG/geometry. Mattress/bedframe: flip `pos_active` (the existing catalog gate). |
...
## 2 · Data model (migration `0160_one_shot_skus.sql`)
...
- ⚠️ **Verify 0160 is still free at implementation time** — the ledger has dup-numbered migrations;
  check the actual disk files + DB objects, don't trust `list_migrations`.
...
| `code` | `{MODEL}-{COMPARTMENT}-{REMARK_SLUG}`, uppercased; collision → `-2/-3` (§3.3) |
...
oneShotCode(modelCode, compartment, slug): `${MODEL}-${COMPARTMENT}-${slug}`.toUpperCase()
```

- `COMPARTMENT` is the canonical parens code (`1A(LHF)`); for mattress/bedframe it is the size code
  (`(Q)`), matching the base SKU's code shape.
- **Collision**: the server checks `mfg_products.code` for the candidate; if taken, append `-2`,
  `-3`, … until free (one batched `SELECT … WHERE code = ANY(...)` then suffix in memory — no
  per-candidate round-trip; respects the CF 50-subrequest cap).
...
1. Add the custom **compartment moduleId** — `moduleCodeFromSku(code, baseModel)` =
   `1A(LHF)-SEAT-EXTEND-40CM` — to `product_models.allowed_options.compartments`.
2. Seed `maintenanceConfigHistory.config.sofaCompartmentMeta[normalizedCustomCode]` =
   `{ imageKey: <base compartment imageKey, e.g. 'sofa-modules/1A(LHF).png'>, description: <remark> }`.
3. Set the SKU `pos_active = true`; set `sell_price_sen` to the D9 re-activation price.
4. **Palette extension** (the sanctioned deviation, D10): `apps/pos/src/pages/CustomBuilder.tsx`
   palette (~1030–1097) renders, in addition to static `SOFA_MODULES`, the model's custom
   compartments that resolve via `synthesizeModule`/`parseCompartmentStructure`
   (sofa-build.ts:229,265) — base family `1A` + orientation `LHF` recovered → **base geometry**.
   `resolveModuleArtSrc` (CustomBuilder.tsx:333–342) resolves to the base PNG via the seeded
   `imageKey`; add a **structural art fallback** (resolve the base representative's art when a direct
   asset 404s) as belt-and-suspenders.
...
code change**. Snap math, the 22 PNGs, drag — all untouched.
...
1. Migration 0160 via Supabase MCP (additive — safe before code), flag default OFF.
