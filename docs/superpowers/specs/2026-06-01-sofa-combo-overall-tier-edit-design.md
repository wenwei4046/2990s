...
### 6.1 Storage — `sofa_combo_tier_offsets` (new, migration `0124`)

Mirror the proven `delivery_fee_config` single-row pattern (one global row, `id = 1`).

```sql
-- 0124_sofa_combo_tier_offsets.sql
...
- Migration number `0124` — main shipped `0122_reset_test_transactions_keep_so_fn` and the parallel `feat/sofa-pool-final` branch took `0123` (both on prod), so this lands at `0124`. The repo also tolerates duplicate numbers (two `0118_*` exist).
...
- Migration `0124` applied to **prod first + verified** (read-back the seeded `id=1` row) before deploy, per project practice for pricing tables.
...
- **Migration number:** renumbered `0122→0123→0124` as concurrent sections claimed numbers (main's `0122_reset`, sofa-pool's `0123`); applied to prod as `0124` (verified).
