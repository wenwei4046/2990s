...
/* ── Combo Price 1/2/3 by fabric tier (Option B, migration 0179) ──────────────
   A combo can carry EXPLICIT per-tier selling maps (price2_by_height /
   price3_by_height). When a build's fabric resolves to PRICE_2 / PRICE_3 and the
   map is non-empty, the recompute charges that explicit combo price AND
   suppresses the flat fabric-tier add-on (the tier premium is baked into the
   map). A combo WITHOUT those maps prices EXACTLY as before (price1 + flat
   add-on) — the backward-compat guarantee. */
describe('recomputeFromSnapshot — Combo Price 1/2/3 (Option B, migration 0179)', () => {
  const ADDON_CFG = { sofaTier2Delta: 150, sofaTier3Delta: 250, bedframeTier2Delta: 200, bedframeTier3Delta: 300 };
  // Single-module build + single-slot combo so the combo deterministically
  // matches the lone group (mirrors the PWP test fixture).
  const oneCell = { cells: [{ id: 'a', moduleId: '1A(LHF)', x: 0, y: 0, rot: 0 }], depth: '24' } as unknown as null;
  const MODULE_PRICE = { '1A(LHF)': 150000 };
  // price1 RM2000; price2 RM2400 (explicit); price3 left unset.
  const tierCombo: SofaComboRow = {
    id: 'cmb-tier', baseModel: 'Booqit',
    modules: [['1A(LHF)']],
    tier: 'PRICE_1', customerId: null,
    pricesByHeight: { '24': 200000 },          // price1 charged (merged)
    sellingPricesByHeight: { '24': 200000 },   // raw price1 selling
    costPricesByHeight: { '24': 150000 },       // raw cost
    price2ByHeight: { '24': 240000 },           // EXPLICIT tier-2 selling
    price3ByHeight: {},                          // unset → tier-3 inherits price1 + add-on
    effectiveFrom: '2020-01-01', deletedAt: null,
  };
  // A legacy combo with NO tier maps — must price byte-identically to pre-Option-B.
  const legacyCombo: SofaComboRow = {
    id: 'cmb-legacy', baseModel: 'Booqit',
    modules: [['1A(LHF)']],
    tier: 'PRICE_1', customerId: null,
    pricesByHeight: { '24': 200000 },
    sellingPricesByHeight: { '24': 200000 },
    costPricesByHeight: { '24': 150000 },
    effectiveFrom: '2020-01-01', deletedAt: null,
  };

  it('PRICE_1 fabric → charges combo price1 (RM2000), no add-on', () => {
    const r = recomputeFromSnapshot(
      { itemCode: 'BOOQIT-1A', itemGroup: 'sofa', qty: 1, unitPriceCenti: 200000, variants: oneCell },
      sofaProduct, null, null, [tierCombo], MODULE_PRICE,
      { sofaTier: 'PRICE_1', bedframeTier: null }, ADDON_CFG,
    );
    expect(r.drift).toBe(false);
    expect(r.unit_price_sen).toBe(200000);
  });

  it('PRICE_2 fabric with explicit price2 → charges price2 (RM2400), flat add-on SUPPRESSED', () => {
    const r = recomputeFromSnapshot(
      { itemCode: 'BOOQIT-1A', itemGroup: 'sofa', qty: 1, unitPriceCenti: 240000, variants: oneCell },
      sofaProduct, null, null, [tierCombo], MODULE_PRICE,
      { sofaTier: 'PRICE_2', bedframeTier: null }, ADDON_CFG,
    );
    expect(r.drift).toBe(false);
    // 240000 (explicit price2) and NOT 200000 + 15000 flat add-on.
    expect(r.unit_price_sen).toBe(240000);
  });

  it('PRICE_3 fabric with NO price3 map → falls back to price1 + flat add-on (RM2000 + RM250)', () => {
    const r = recomputeFromSnapshot(
      { itemCode: 'BOOQIT-1A', itemGroup: 'sofa', qty: 1, unitPriceCenti: 225000, variants: oneCell },
      sofaProduct, null, null, [tierCombo], MODULE_PRICE,
      { sofaTier: 'PRICE_3', bedframeTier: null }, ADDON_CFG,
    );
    expect(r.drift).toBe(false);
    expect(r.unit_price_sen).toBe(225000); // 200000 price1 + 25000 flat add-on (NOT suppressed)
  });

  it('BACKWARD-COMPAT: a combo with no tier maps still gets the flat add-on (RM2000 + RM150)', () => {
    const r = recomputeFromSnapshot(
      { itemCode: 'BOOQIT-1A', itemGroup: 'sofa', qty: 1, unitPriceCenti: 215000, variants: oneCell },
      sofaProduct, null, null, [legacyCombo], MODULE_PRICE,
      { sofaTier: 'PRICE_2', bedframeTier: null }, ADDON_CFG,
    );
    expect(r.drift).toBe(false);
    expect(r.unit_price_sen).toBe(215000); // 200000 + 15000 flat add-on — identical to pre-Option-B
  });

  it('anti-tamper: PRICE_2 explicit combo, client omits to price1 → drift true', () => {
    const r = recomputeFromSnapshot(
      { itemCode: 'BOOQIT-1A', itemGroup: 'sofa', qty: 1, unitPriceCenti: 200000, variants: oneCell },
      sofaProduct, null, null, [tierCombo], MODULE_PRICE,
      { sofaTier: 'PRICE_2', bedframeTier: null }, ADDON_CFG,
    );
    expect(r.drift).toBe(true); // server 240000 vs client 200000 → >0.5%
  });
});
