...
const HEADER =
  'id, pc_number, supplier_invoice_ref, supplier_id, purchase_order_id, grn_id, consignment_date, due_date, currency, subtotal_centi, tax_centi, total_centi, paid_centi, status, notes, posted_at, created_at, created_by, updated_at';
const ITEM =
  'id, purchase_consignment_id, grn_item_id, material_kind, material_code, material_name, qty, unit_price_centi, line_total_centi, notes, ' +
...
const nextNum = async (sb: any, prefix: string): Promise<string> => {
  ...
  const { count } = await sb.from('purchase_consignments').select('id', { head: true, count: 'exact' }).like('pc_number', `${prefix}-${yymm}-%`);
  return `${prefix}-${yymm}-${String((count ?? 0) + 1).padStart(3, '0')}`;
};

/* ── Recompute PI header money rollups (mirror recomputeGrnTotals) ─────────
   Sum line_total_centi across purchase_consignment_items → write subtotal_centi,
...
async function recomputePiTotals(sb: any, piId: string) {
  const [itemsRes, headerRes] = await Promise.all([
    sb.from('purchase_consignment_items').select('line_total_centi').eq('purchase_consignment_id', piId),
    sb.from('purchase_consignments').select('tax_centi').eq('id', piId).maybeSingle(),
  ]);
  ...
  await sb.from('purchase_consignments').update({
...
async function piLocked(sb: any, piId: string): Promise<{ error: string; message: string } | null> {
  const { data } = await sb.from('purchase_consignments')
...
purchaseConsignments.get('/', async (c) => {
  const sb = c.get('supabase');
  let q = sb.from('purchase_consignments')
  ...
  return c.json({ purchaseConsignments: data ?? [] });
});
...
purchaseConsignments.get('/outstanding-grn-items', async (c) => {
  const sb = c.get('supabase');
...
purchaseConsignments.get('/:id', async (c) => {
  const sb = c.get('supabase'); const id = c.req.param('id');
  const [h, i] = await Promise.all([
    sb.from('purchase_consignments').select(`${HEADER}, supplier:suppliers(id, code, name)`).eq('id', id).maybeSingle(),
    sb.from('purchase_consignment_items').select(ITEM).eq('purchase_consignment_id', id).order('created_at'),
  ]);
  if (h.error) return c.json({ error: 'load_failed', reason: h.error.message }, 500);
  if (!h.data) return c.json({ error: 'not_found' }, 404);
  return c.json({ purchaseConsignment: h.data, items: i.data ?? [] });
});

// ── Linked docs (Smart Buttons fan-out) ─────────────────────────────
// For a PI: the parent GRN + parent PO (both via FK on purchase_consignments).
purchaseConsignments.get('/:id/linked', async (c) => {
  const sb = c.get('supabase'); const id = c.req.param('id');
  const { data, error } = await sb
    .from('purchase_consignments')
    .select(`
      id,
...
purchaseConsignments.post('/', async (c) => {
  ...
  const pcNumber = await nextNum(sb, 'PURCHASE_CONSIGNMENT');
  let subtotal = 0;
  const itemRows = items.map((it) => {
    ...
    return {
      ...
      // path). Columns exist on purchase_consignment_items (migration 0057).
  ...
  const { data: header, error: hErr } = await sb.from('purchase_consignments').insert({
    pc_number: pcNumber,
...
purchaseConsignments.patch('/:id/post', async (c) => {
  /* PR-DRAFT-removal — kept for backward compat; idempotent. POST now
     creates PIs as POSTED directly. */
  const sb = c.get('supabase'); const id = c.req.param('id');
  const { data: cur } = await sb.from('purchase_consignments').select('id, status').eq('id', id).maybeSingle();
  if (!cur) return c.json({ error: 'not_found' }, 404);
  if ((cur as { status: string }).status === 'POSTED') return c.json({ purchaseConsignment: cur });
...
purchaseConsignments.patch('/:id/payment', async (c) => {
  const sb = c.get('supabase'); const id = c.req.param('id');
  ...
  const { data: cur } = await sb.from('purchase_consignments').select('paid_centi, total_centi, status').eq('id', id).maybeSingle();
  if (!cur) return c.json({ error: 'not_found' }, 404);
...
/* ── POST /from-grn-items ───────────────────────────────────────────────
   Body: { picks: [{ grnItemId, qty }], supplierInvoiceNumber?, consignmentDate?,
           notes? }.
   Server logic:
   ...
   Returns { created: [{ id, pcNumber, supplierId, grnCount, lineCount }], total }. */
purchaseConsignments.post('/from-grn-items', async (c) => {
  const sb = c.get('supabase'); const user = c.get('user');
  let body: {
  ...
  const d = new Date();
  const yymm = `${String(d.getFullYear()).slice(2)}${String(d.getMonth() + 1).padStart(2, '0')}`;
  const { count } = await sb.from('purchase_consignments').select('id', { head: true, count: 'exact' }).like('pc_number', `PI-${yymm}-%`);
  let counter = count ?? 0;

  const consignmentDate = body.consignmentDate ?? new Date().toISOString().slice(0, 10);
  const created: Array<{ id: string; pcNumber: string; supplierId: string; grnCount: number; lineCount: number }> = [];

  for (const bucket of buckets.values()) {
    counter += 1;
    const pcNumber = `PI-${yymm}-${String(counter).padStart(3, '0')}`;
    ...
    const { data: header, error: hErr } = await sb.from('purchase_consignments').insert({
      ...
      created_by: user.id,
    }).select('id, pc_number').single();
    if (hErr) continue;
    const h = header as unknown as { id: string; pc_number: string };

    const rows = bucket.lines.map(({ row, qty }) => ({
      purchase_consignment_id: h.id,
      grn_item_id: row.id,
    ...
    const { error: iErr } = await sb.from('purchase_consignment_items').insert(rows);
    if (iErr) {
      await sb.from('purchase_consignments').delete().eq('id', h.id);
      continue;
    }
...
/* ── POST /from-grn ─────────────────────────────────────────────────────
   ...
   Body: { grnId }  →  201 { id, pcNumber }. */
purchaseConsignments.post('/from-grn', async (c) => {
  const sb = c.get('supabase'); const user = c.get('user');
  ...
  const { data: header, error: hErr } = await sb.from('purchase_consignments').insert({
    ...
    grn_id: g.id,
    consignment_date: new Date().toISOString().slice(0, 10),
    currency: 'MYR',
    subtotal_centi: subtotal,
    tax_centi: 0,
    total_centi: subtotal,
  ...
  const { error: insErr } = await sb.from('purchase_consignment_items').insert(rows);
  if (insErr) { await sb.from('purchase_consignments').delete().eq('id', h.id); return c.json({ error: 'items_insert_failed', reason: insErr.message }, 500); }
...
purchaseConsignments.patch('/:id', async (c) => {
  ...
  for (const [from, to] of [
    ...
    ['consignmentDate', 'consignment_date'], ['dueDate', 'due_date'],
  ...
  const sb = c.get('supabase');
  const { data, error } = await sb.from('purchase_consignments').update(updates).eq('id', id).select(HEADER).single();
  if (error) return c.json({ error: 'update_failed', reason: error.message }, 500);
  return c.json({ purchaseConsignment: data });
});

/* ── POST /:id/items — add one purchase_invoice_item. qty maps to qty. ── */
purchaseConsignments.post('/:id/items', async (c) => {
  ...
  const { data, error } = await sb.from('purchase_consignment_items').insert(row).select(ITEM).single();
...
purchaseConsignments.patch('/:id/items/:itemId', async (c) => {
  ...
  const { data: prev } = await sb.from('purchase_consignment_items')
  ...
  const { error } = await sb.from('purchase_consignment_items').update(updates).eq('id', itemId);
  if (error) return c.json({ error: 'update_failed', reason: error.message }, 500);
...
purchaseConsignments.delete('/:id/items/:itemId', async (c) => {
  const piId = c.req.param('id'); const itemId = c.req.param('itemId');
  const sb = c.get('supabase');
  // PI edit-lock: a paid / cancelled PI is read-only.
  const lock = await piLocked(sb, piId);
  ...
  const { data: line } = await sb.from('purchase_consignment_items')
    .select('qty, grn_item_id').eq('id', itemId).maybeSingle();
  const { error } = await sb.from('purchase_consignment_items').delete().eq('id', itemId);
