...
async function loadActiveSofaCombos(sb: any): Promise<SofaComboRow[]> {
  const { data } = await sb
    ...
    .select('id, base_model, modules, tier, customer_id, prices_by_height, selling_prices_by_height, pwp_prices_by_height, price2_by_height, price3_by_height, label, effective_from, deleted_at, default_free_gifts')
    .is('deleted_at', null)
