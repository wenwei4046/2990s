...
purchaseConsignmentReturns.post('/', async (c) => {
  ...
  /* Hard reset 2026-05-30: PCR POST writes inventory OUT (you ship consignment
     stock back to supplier — stock leaves warehouse). Mirrors PR POST. */
  ...
  if (warehouseId) {
    await writeMovements(sb, itemRows.filter((r) => r.qty > 0).map((r) => ({
      ...
      qty: r.qty,
      /* OUT doesn't carry unit_cost_sen — FIFO trigger computes COGS from
         consumed lots (matches PR + DO POST pattern). */
      source_doc_type: 'PURCHASE_CONSIGNMENT_RETURN' as const,
      source_doc_id: h.id,
      source_doc_no: h.pcr_number,
...
purchaseConsignmentReturns.patch('/:id/cancel', async (c) => {
  ...
  const { data: cur } = await sb.from('purchase_consignment_returns')
    .select('id, status, refund_centi').eq('id', id).maybeSingle();
  if (!cur) return c.json({ error: 'not_found' }, 404);
  const head = cur as { id: string; status: string; refund_centi: number | null };
  if ((head.refund_centi ?? 0) > 0) {
    return c.json({ error: 'cannot_cancel', message: 'PCR already refunded' }, 409);
  }
  ...
  /* Hard reset 2026-05-30: reverse the inventory OUT written at POST.
     reverseMovements is signed-net per bucket — idempotent on re-call. */
  try { await reverseMovements(sb, 'PURCHASE_CONSIGNMENT_RETURN', id, user.id); } catch { /* best-effort */ }
