...
// ── POST /copy-to-customer ─────────────────────────────────────────────
// Duplicate ALL currently-active rules from one customer scope (or the
// default null scope) into another customer. Useful when onboarding a new
// B2B customer with the same pricing as an existing one. Each duplicate
// row gets the SAME effectiveFrom as the source (commander expectation:
// no date shift) and the target customerId.
//
// body: { fromCustomerId: uuid | null, toCustomerId: uuid, baseModel? }
sofaCombos.post('/copy-to-customer', async (c) => {
  let body: { fromCustomerId?: string | null; toCustomerId?: string; baseModel?: string };
  try {
    body = (await c.req.json()) as typeof body;
  } catch {
    return c.json({ error: 'invalid_json' }, 400);
  ...
  const toCustomerId = (body.toCustomerId ?? '').trim();
  if (!toCustomerId) return c.json({ error: 'to_customer_required' }, 400);

  const fromCustomerId =
    body.fromCustomerId === null || body.fromCustomerId === undefined || body.fromCustomerId === ''
      ? null
      : body.fromCustomerId;
  ...
  let q = supabase
    .from('sofa_combo_pricing')
    .select('base_model, modules, tier, prices_by_height, label, effective_from, notes')
    .is('deleted_at', null);

  if (fromCustomerId === null) q = q.is('customer_id', null);
  else q = q.eq('customer_id', fromCustomerId);

  if (body.baseModel) q = q.eq('base_model', body.baseModel);

  const { data: srcRows, error: srcErr } = await q;
  if (srcErr) return c.json({ error: 'load_failed', reason: srcErr.message }, 500);
  ...
  for (const r of (srcRows ?? []) as Array<{
    ...
    if (r.effective_from > today) continue;
    const key = JSON.stringify([r.base_model, normalizeComboModules(r.modules), r.tier]);
    if (seen.has(key)) continue;
    seen.add(key);
    sources.push(r);
  }

  if (sources.length === 0) return c.json({ copied: 0 });

  const user = c.get('user');
  const inserts = sources.map((r) => ({
    base_model: r.base_model,
    modules: r.modules,
    tier: r.tier,
    customer_id: toCustomerId,
    prices_by_height: r.prices_by_height ?? {},
    label: r.label,
  ...
  return c.json({ copied: ins?.length ?? 0 });
});
