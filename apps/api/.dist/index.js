...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260508.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260508.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
  ...
  */
  async processResponse(res) {
    ...
    if (res.ok) {
      ...
      if (error3 && _this2.shouldThrowOnError)
...
var Socket3 = /* @__PURE__ */ __name(class {
  ...
  */
  ...
  onConnError(error3) {
    if (this.hasLogger())
      this.log("transport", "error", error3);
...
var supabaseAuth = createMiddleware(
  async (c, next) => {
...
products.get("/", async (c) => {
  ...
  const { data, error: error3 } = await supabase.from("products").select(
    `
    ...
    `
  ).eq("visible", true).order("updated_at", { ascending: false });
  if (error3)
