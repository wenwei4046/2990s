...
// 'pr-g.cn-list.filter-order.v1') — never reuse the DO/SO/DR keys.
...
const FILTER_ORDER_KEY = 'pr-g.cn-list.filter-order.v1';
...
const DraggableFilter = ({
  ...
  const onDragStart = (e: DragEvent<HTMLDivElement>) => {
    e.dataTransfer.setData('text/x-cn-filter', id);
    e.dataTransfer.effectAllowed = 'move';
  };
  const onDragOver = (e: DragEvent<HTMLDivElement>) => { e.preventDefault(); setOver(true); };
  const onDrop = (e: DragEvent<HTMLDivElement>) => {
    e.preventDefault(); setOver(false);
    const src = e.dataTransfer.getData('text/x-cn-filter') as FilterId;
...
export const ConsignmentNotes = () => {
  ...
  const rows = useMemo<CnRow[]>(() => {
    ...
    return allRows.filter((r) => {
      ...
      if (q) {
        const blob = [
          r.do_number, r.debtor_name, r.debtor_code, r.venue,
          deriveBranding(r), r.customer_so_no, r.ref, r.phone, r.driver_name,
                ...
                <DraggableFilter key={fid} id={fid} order={filterOrder} setOrder={setFilterOrder}>
                  <div style={{ position: 'relative', flex: '1 1 240px', minWidth: 200, display: 'inline-block' }}>
                    ...
                    <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
                      placeholder="Note No, debtor, driver…"
