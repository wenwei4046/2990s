...
const useSoPaymentsForDrilldown = (docNo: string | null) => useQuery({
  ...
  queryFn: async () => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL}/consignment-orders/${docNo}/payments`, {
...
export const ConsignmentOrders = () => {
  ...
  const onNew = () => navigate('/consignment/new');
  const openDetail = (row: SoRow, edit = false) =>
    navigate(`/consignment/${row.doc_no}${edit ? '?edit=1' : ''}`);
  ...
  const renderPdf = async (row: SoRow, action: 'save' | 'print' | 'preview') => {
    ...
    const [detailRes, paymentsRes] = await Promise.all([
      fetch(`${import.meta.env.VITE_API_URL}/consignment-orders/${row.doc_no}`, { headers }),
      fetch(`${import.meta.env.VITE_API_URL}/consignment-orders/${row.doc_no}/payments`, { headers }),
    ]);
  ...
  const convertToDo = (row: SoRow) => {
    ...
    if (!row.has_undelivered || ['CANCELLED', 'CLOSED', 'ON_HOLD'].includes(row.status)) {
      window.alert('Nothing to be converted — every line on this Consignment Order is already on a note (or the order is closed / cancelled / on hold).');
      return;
    }
    navigate(`/consignment-note/new?fromConsignmentOrder=${encodeURIComponent(row.doc_no)}`);
  };
  ...
  const copyToNewSo = (row: SoRow) => {
    navigate(`/consignment/new?copyFrom=${encodeURIComponent(row.doc_no)}`);
  };
