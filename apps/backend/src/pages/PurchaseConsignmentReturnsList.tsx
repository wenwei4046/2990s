// ----------------------------------------------------------------------------
// PurchaseConsignmentReturnsList — Purchase Consignment Return (PI) list, cloned from the polished
...
//
// A Purchase Consignment Return is born from a Goods Receipt: the user converts a GRN
// (right-click "Convert to PI" on the GRN list, or the "From GRN" button here
// routes to /grns). A manual New Invoice page exists at /purchase-consignment-returns/new.
// ----------------------------------------------------------------------------
...
import { usePurchaseConsignmentReturns, useCancelPurchaseConsignmentReturn } from '../lib/flow-queries';
...
const buildPiColumns = (): DataGridColumn<PiRow>[] => [
  {
    key: 'pcr_number', label: 'Invoice No.', width: 150, sortable: true,
    accessor: (r) => <span className={styles.codeChip}>{r.pcr_number}</span>,
    searchValue: (r) => r.pcr_number,
    sortFn: (a, b) => a.pcr_number.localeCompare(b.pcr_number),
  },
  {
...
export const PurchaseConsignmentReturns = () => {
  ...
  const cancelPi = useCancelPurchaseConsignmentReturn();
  ...
  const doCancelPi = (r: PiRow) => {
    if (!window.confirm(`Cancel invoice ${r.pcr_number}? This sets status to CANCELLED — line items stay for audit.`)) return;
    ...
    <div className={styles.page}>
      <div className={styles.headerRow}>
        ...
        <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
          {/* A Purchase Consignment Return starts from a Goods Receipt — this routes to the
              GRN list where the user right-clicks "Convert to PI". */}
          <Button variant="ghost" size="md" onClick={() => navigate('/grns')}>
            <FileText {...ICON} />
            <span>From GRN</span>
          </Button>
          <Button variant="primary" size="md" onClick={() => navigate('/purchase-consignment-returns/new')}>
            <Plus {...ICON} />
      ...
      <DataGrid<PiRow>
        ...
        onRowDoubleClick={(r) => navigate(`/purchase-consignment-returns/${r.id}`)}
        ...
        contextMenu={(r) => {
          ...
          const menu: Array<{ label?: string; onClick?: () => void; danger?: boolean; divider?: true }> = [
            { label: 'View', onClick: () => navigate(`/purchase-consignment-returns/${r.id}`) },
          ];
          if (!locked) {
            menu.push({ label: 'Edit', onClick: () => navigate(`/purchase-consignment-returns/${r.id}?edit=1`) });
