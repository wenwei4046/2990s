// ----------------------------------------------------------------------------
// PurchaseConsignmentDetail — full-page route at /purchase-consignments/:id.
//
...
import {
  ...
  useUpdatePurchaseConsignmentHeader,
  useUpdatePurchaseConsignmentItem,
  useDeletePurchaseConsignmentItem,
  useCancelPurchaseConsignment,
...
export const PurchaseConsignmentDetail = () => {
  ...
  const updateHeader = useUpdatePurchaseConsignmentHeader();
  const updateItem = useUpdatePurchaseConsignmentItem();
  const deleteItem = useDeletePurchaseConsignmentItem();
  const cancel = useCancelPurchaseConsignment();
  ...
  const handlePrint = () => {
    ...
    import('../lib/purchase-invoice-pdf').then(({ generatePurchaseConsignmentPdf }) =>
      generatePurchaseConsignmentPdf(pi, items as any),
        ...
        <div className={styles.titleBlock}>
          ...
          <div>
            <h1 className={styles.title}>
              ...
              {pi.pc_number} — {pi.supplier?.name ?? pi.supplier?.code ?? '—'}
            </h1>
          </div>
        </div>
        <div className={styles.actions}>
          ...
          {!isLocked && (
            <Button variant="ghost" size="md"
              onClick={() => {
                if (!confirm(`Cancel invoice ${pi.pc_number}? This sets status to CANCELLED — line items stay for audit.`)) return;
                cancel.mutate(pi.id, {
