...
export const useSofaCustomizerData = (leadSkuId: string | undefined) =>
  useQuery({
    ...
    queryFn: async (): Promise<SofaCustomizerData | null> => {
      ...
      //
      // ⚠️ A module the catalogue prices as null contributes RM 0 to the build,
      // so Models with unpriced modules quote LOW here and are then refused by
      // Houzs's drift gate at handover ("tablet RM 990 vs server RM 1,980").
      // A 2990-side override table (migration 0213) briefly filled those nulls;
      // it was removed 2026-08-14 in favour of the single drift-adopt step at
      // handover, so the catalogue is once again the only price source. The
      // real fix is complete prices in the Houzs catalogue.
