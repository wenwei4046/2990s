...
// ---------------------------------------------------------------------------
// Toast — in-app notification bubble replacing naked window.alert(...) in the
// POS. Code mirrors the Backend's apps/backend/src/components/Toast.tsx (which
// itself was ported from HOOKKA, commit d2e37ff0). Per the monorepo rule that
// apps/pos and apps/backend must never import each other, the component is
// duplicated here rather than shared — same API surface so both apps look and
// behave identically.
//
// API:
//   const toast = useToast();
//   toast('hi');              // info (legacy callable)
//   toast.success('Saved');
//   toast.error('Failed');
