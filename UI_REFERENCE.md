...
**Status:** Phases 1–3 done; **Phase 4 (combo semantics Q2 + server selling drift-reject Q1) landed** on `feat/cost-sell-split` (not yet deployed). Pending: sofa selling recompute (Phase 4b) + QuickPick (Phase 5). See `COST-SELL-SPLIT-PLAN.md` (locked decisions D1–D8 + phased roadmap). **This deviation modifies §1**: the per-Model SELLING-price editor role moves off the Backend SKU Master onto the POS Master Account; Backend pricing becomes COST-only.
...
- **Server-side (production `/mfg-sales-orders`):** selling price is **recomputed from the Master Account store** (`mfg_products.sell_price_sen`); a client price that drifts **> 0.5% is REJECTED (HTTP 400)** per the CLAUDE.md non-negotiable — Loo's 2026-05-30 ruling (Q1), reversing the earlier "log-only" draft. **Scope:** mattress / bedframe / accessory / service enforced now; **sofa** (compartment + combo across module lines) is recomputed on a later pass (Phase 4b) and trusts the operator price until then; custom lines (no sell price) and a client price of 0 ("not provided") are accepted, not rejected.
