...
- ⚠️ **Do NOT drop the retail `products` TABLE yet.** Still has live readers: (1) `Handover.tsx` via `useCatalog` (delivery-fee cross-category detection + `cartLinesToSoItems` enrichment); (2) `apps/pos/src/lib/queries.ts` `useProduct` non-mfg-UUID fallback. (The third reader, legacy `apps/api/src/routes/orders.ts`, was deleted with the /orders route on 2026-06-12.) In production these read an EMPTY table (no crash, just no-op). Delete the page now; drop the table only after these readers are cleaned up (separate task).
